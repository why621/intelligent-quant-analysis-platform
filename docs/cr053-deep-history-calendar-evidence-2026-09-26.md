...
> **本节的"官方公告原文未取得"已被 CR-054 推翻**：公告就在「休市安排」栏目 6 页里，
> 上一轮只是没读列表页源码找真实分页地址。2015—2024 中已有 9 年按公告原文升级为
> `official-notice`，仅 2020 年保持本节所述的交叉核对口径。见
> [CR-054 实录](cr054-official-notice-calendar-2026-09-26.md)。以下按 CR-053 当轮事实保留。
...
| 事项 | 结果 | 证据 |
| --- | --- | --- |
| 上交所年度休市通知 2015—2024 原文 | 本轮未取得（CR-054 已取得并升级 9 年） | 该栏目为 JS 分页，抓取只返回 2026 年条目；搜索仅命中当年新闻。未猜测 URL |
...
> CR-054 与 CR-055 之后这段输出已整体过期：2015—2024 十年都按公告原文升级为
> `official-notice`（CR-054 九年 + CR-055 的 2020，后者补齐 2020-01-31），同样的请求不再被拒。
> 被拒绝的只剩"没有证据记录的年份"（2015 之前；CR-056 已把 2014 补入，如今的下界是 2013 及更早），
> 而 `cross-validated` 仍是会触发拒绝的口径，
> 只是这张表里暂无实例。见 [CR-054 实录](cr054-official-notice-calendar-2026-09-26.md) 第 6 节、
> [CR-055 实录](cr055-2020-spring-festival-notice-2026-09-26.md) 第 3 节。
...
1. **2015—2024 官方休市通知原文仍未取得**。当前十年是 `cross-validated` 证据，只可用于研究回填；
   ...
   —— **本轮之后已由 CR-054 执行**：9 年升级为 `official-notice` 并保留公告 URL/文号/核对日期，
   2020 年因一处休市日缺公告原文而仍留 `cross-validated`。
