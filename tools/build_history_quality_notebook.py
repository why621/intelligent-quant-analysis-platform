"""Generate and execute an offline CR-013 companion; no source HTTP requests."""
...
def main():
    ...
    notebook.cells = [
        nbf.v4.new_markdown_cell(
            ...
            "6只腾讯真实样本均返回；5只完整242日，688981为241日，缺2025-09-08。"
            "上交所2025-024公告证实该日停牌，9月9日复牌。"
            "自动覆盖器尚不消费停牌证据，故候选仍gaps且不入可用库；不能称全300通过。"
        ),
        nbf.v4.new_markdown_cell(
            "## Context & Methods\n"
            "颗粒度：固定名单版本+股票身份+qfq+区间。主键：symbol/adjust/trade_date。"
            "检查来源哈希、完整Schema、交易日集合、重复与非法OHLCV、金额空值、"
            "隔离库重读及零网络重放一致性。\n### Key Assumptions\n"
            "当前名单用于历史研究有幸存者偏差；242日是市场交易日，不等于每只股票可交易日。"
            "观察文件是SDK与项目适配后的数据，不是上游原始HTTP字节；"
            "基本质量检查不证明除权复权经济正确、量额单位跨源一致或许可。"
        ),
        nbf.v4.new_markdown_cell(
            "## Data\n只读artifacts/cr013-history-20260908与其离线重放。"
            "合成测试不作为真实覆盖证据；不触及生产数据库。"
        ),
        ...
        nbf.v4.new_markdown_cell(
            "## Takeaways\n"
            "当前已验证5/300完整市场交易日覆盖，1只缺口已获得停牌解释，294只未采集。"
            "下一步实现证据化停牌/上市/代码变更分类与研究可交易日约束，再分批扩大；"
            "不能填价、缩小分母或把停牌当成服务器故障。"
            "本批只采集一次、一个环境，不能判断跨日稳定性或历史版本漂移。\n"
            "官方停复牌证据（人工核查，未被上述代码自动导入）："
            "[中芯国际2025-024公告，2025-09-09](https://star.sse.com.cn/disclosure/listedinfo/announcement/c/new/2025-09-09/688981_20250909_2BQ9.pdf)。"
        ),
    ]
    nbf.validate(notebook)
    target = root / "docs/notebooks/cr013-history-quality.ipynb"
    nbf.write(notebook, target)
    executed = NotebookClient(notebook, timeout=60, kernel_name="python3",
                              resources={"metadata": {"path": str(root)}}).execute()
    output = root / "artifacts/cr013-notebook"
